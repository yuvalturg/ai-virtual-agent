...
from ..models import ProductCreate, ProductOrderRequest, Order as PydanticOrder, Product as PydanticProduct
...
async def db_session() -> AsyncSession:
    ...
    async with database.engine.begin() as conn:
        await conn.run_sync(database.Base.metadata.drop_all) # Drop all first for clean state
...
async def test_add_product_and_get_by_id(db_session: AsyncSession):
    ...
    product_in = ProductCreate(name="Test Book", description="A book for testing", inventory=10, price=19.99)
    ...
    assert float(created_product_db.price) == product_in.price # Compare as float if Numeric

    retrieved_product_db = await crud.get_product_by_id(db_session, product_id=created_product_db.id)
...
async def test_order_product_successful(db_session: AsyncSession):
    ...
    product_in = ProductCreate(name="Orderable Item", description="In stock", inventory=5, price=10.00)
    product_db = await crud.add_product(db_session, product=product_in)
    await db_session.commit()

    order_details = ProductOrderRequest(product_id=product_db.id, quantity=2, customer_identifier="cust123")
    ...
    updated_product_db = await crud.get_product_by_id(db_session, product_id=product_db.id)
...
async def test_order_product_insufficient_inventory(db_session: AsyncSession):
    ...
    product_in = ProductCreate(name="Limited Stock Item", description="Low stock", inventory=1, price=25.00)
    product_db = await crud.add_product(db_session, product=product_in)
    await db_session.commit()

    order_details = ProductOrderRequest(product_id=product_db.id, quantity=2, customer_identifier="cust456")
    
    with pytest.raises(ValueError, match=f"Insufficient inventory for product {product_db.name}"):
    ...
    check_product_db = await crud.get_product_by_id(db_session, product_id=product_db.id)
...
async def test_order_product_not_found(db_session: AsyncSession):
    ...
    order_details = ProductOrderRequest(product_id=999, quantity=1, customer_identifier="cust789")
...
async def test_search_products(db_session: AsyncSession):
    ...
    await crud.add_product(db_session, ProductCreate(name="Alpha Game", inventory=10, price=59.99))
    await crud.add_product(db_session, ProductCreate(name="Beta Game Console", inventory=5, price=299.99))
    await crud.add_product(db_session, ProductCreate(name="Alpha Accessories", inventory=20, price=19.99))
    await db_session.commit()
