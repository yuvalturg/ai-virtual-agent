...
from .. import database, crud # For direct DB manipulation/verification in tests
from ..models import ProductCreate, ProductOrderRequest # Pydantic models for creating data

@pytest_asyncio.fixture(scope="module") # Module scope: server & DB setup once per test file
...
async def test_tool_add_product_and_get_products(initialized_test_mcp_server):
    ...
    async with Client(initialized_test_mcp_server) as client:
        ...
        add_payload = {"name": "Tool Test Widget", "description": "A widget added via tool", "inventory": 20, "price": 9.99}
        ...
        assert any(p["name"] == "Tool Test Widget" and p["id"] == added_product_data["id"] for p in products)

        # Test get_product_by_id tool
        get_by_id_response = await client.call_tool("get_product_by_id", {"product_id": added_product_data["id"]})
        assert get_by_id_response.ok
        assert get_by_id_response.content["name"] == "Tool Test Widget"

        # Test get_product_by_name tool
        get_by_name_response = await client.call_tool("get_product_by_name", {"name": "Tool Test Widget"})
...
async def test_tool_order_product_insufficient_inventory(initialized_test_mcp_server):
    ...
    async with Client(initialized_test_mcp_server) as client:
        ...
        add_payload = {"name": product_name, "description": "Limited for eval", "inventory": 1, "price": 50.0}
        add_response = await client.call_tool("add_product", add_payload)
        assert add_response.ok
        product_id_to_order = add_response.content["id"]

        # 2. Attempt to order more than available
        order_payload = {"product_id": product_id_to_order, "quantity": 2, "customer_identifier": "eval_user_001"}
        ...
        verify_response = await client.call_tool("get_product_by_id", {"product_id": product_id_to_order})
...
async def test_tool_search_products(initialized_test_mcp_server):
    ...
    async with Client(initialized_test_mcp_server) as client:
        ...
        await client.call_tool("add_product", {"name": "Search Alpha One", "inventory": 5, "price": 1.0})
        await client.call_tool("add_product", {"name": "Search Beta Two", "inventory": 5, "price": 2.0})
        await client.call_tool("add_product", {"name": "Another Alpha Item", "inventory": 5, "price": 3.0})

        search_alpha_response = await client.call_tool("search_products", {"query": "Alpha"})
        ...
        search_beta_response = await client.call_tool("search_products", {"query": "Beta"})
