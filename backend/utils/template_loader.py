...
def load_all_templates_from_directory(templates_dir: str = "agent_templates"):
    ...
    for yaml_file in yaml_files:
        try:
            ...
            templates = {}
            logger.info(f"Processing {len(suite_config.get('templates', {}))} templates in suite {suite_id}")
            for template_id, template_config in suite_config.get("templates", {}).items():
                logger.debug(f"Converting template: {template_id}")
                agent_template = convert_yaml_template_to_agent_template(template_config)
    ...
    logger.info(f"Loaded {len(all_suites)} suites with {len(all_templates)} total templates")
