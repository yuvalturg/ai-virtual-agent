...
def upgrade() -> None:
    op.drop_constraint('chat_history_virtual_assistant_id_fkey', 'chat_history', type_='foreignkey')
...
def downgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("virtual_assistant_id", sa.UUID(as_uuid=True), sa.ForeignKey("virtual_assistants.id", ondelete="CASCADE"), primary_key=True),
        sa.Column("vector_db_name", sa.String(length=255), sa.ForeignKey("knowledge_bases.vector_db_name"), primary_key=True),
    )
    op.create_table(
    ...
    op.create_foreign_key('chat_history_virtual_assistant_id_fkey', 'chat_history', 'virtual_assistants', ['virtual_assistant_id'], ['id'], ondelete='CASCADE')
