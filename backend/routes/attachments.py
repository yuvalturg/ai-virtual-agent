...
MINIO_ACCESS_KEY = os.getenv("MINIO_ACCESS_KEY", "minio_rag_user")
MINIO_SECRET_KEY = os.getenv("MINIO_SECRET_KEY", "minio_rag_password")
BUCKET_NAME = os.getenv("BUCKET_NAME", "attachments")
...
async def upload_attachment(session_id: str = Form(...), file: UploadFile = File(...)):
    ...
    try:
        ...
        if file.filename:
            ...
            # for this session_id.
            s3client.upload_fileobj(file.file, BUCKET_NAME, f"{session_id}/{attachment_id}{ext}")
...
async def get_attachment(session_id: str, attachment: str):
    ...
    try:
        ...
        # for this session_id.
        s3client.download_fileobj(BUCKET_NAME, f"{session_id}/{attachment}", fileobj)
        fileobj.seek(0)
