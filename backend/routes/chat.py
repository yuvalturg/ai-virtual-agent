...
class Chat:
    ...
    Methods:
        stream: Streams the chatbot's response based on agent_id, session_id, and prompt.
    """
    ...
    def _get_agent_config(self, agent_id: str):
        ...
        try:
            agent_config = self._get_client().agents.get(agent_id=agent_id)
            return agent_config
    ...
    def _get_tools_for_agent(self, agent_id: str):
        ...
        try:
            ...
            if agent_config and hasattr(agent_config, 'tool_config') and agent_config.tool_config:
    ...
    def _response_generator(self, turn_response, session_id: str, agent_type: AgentType):
        if agent_type == AgentType.REACT:
        ...
        for response in turn_response:
            ...
            if payload.event_type == "step_complete":
                ...
                if step_details.step_type == "inference":
                    for chunk in self._process_inference_step_json(current_step_content, tool_results, final_answer):
                        yield chunk
                    current_step_content = ""
    ...
    def _process_inference_step_json(self, current_step_content, tool_results, final_answer):
        ...
        try:
            ...
            yield json.dumps({
                "type": "error",
                "content": f"Failed to parse ReAct step content: {current_step_content}"
            })
        except Exception as e:
            yield json.dumps({
                "type": "error",
                "content": f"Failed to process ReAct step: {e} - {current_step_content}"
            })
    ...
    def _format_tool_results_summary_json(self, tool_results):
        ...
        for tool_name, content in tool_results:
            try:
                ...
                summary_text += f"\n**{tool_name}** was used but encountered an error: {type(e).__name__}\n"
        ...
        for response in turn_response:
            if hasattr(response.event, "payload"):
                ...
                if response.event.payload.event_type == "step_complete":
                    ...
                    #         yield json.dumps({"type": "end", "content": "end_of_turn"})
                ...
                yield json.dumps({
                    "type": "error",
                    "content": f"Error occurred in the Llama Stack Cluster: {response}"
                })
    ...
    def stream(self, agent_id: str, session_id: str, prompt: str):
        ...
        try:
            ...
            # Note: LlamaStack manages session state, so we don't need to maintain local state
            ...
            self.log.error(f"Error in stream for agent {agent_id}, session {session_id}: {e}")
            yield json.dumps({
                "type": "error",
